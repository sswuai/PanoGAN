...
class panoganBaseline4eModel(BaseModel):
    """
    ...
    def forward(self):
        ...
        self.fake_B_final = fake_B
        self.fake_D_final = fake_D
    ...
    def backward_D(self):
        ...
        # ---------img loss ------------------------
        # fake
        fake_AB = torch.cat((self.img_A, self.fake_B_final),
    ...
    # ---------seg loss ------------------------
    # fake
        fake_AD = torch.cat((self.img_A, self.fake_D_final),
                            1)  # we use conditional GANs; we need to feed both input and output to the discriminator
        ...
        self.loss_D = (self.loss_D_fake_img + self.loss_D_real_img +
                           self.loss_D_fake_seg + self.loss_D_real_seg)  * 0.5
    ...
    def backward_G(self):
        ...
        #--------L1 loss--------------------------
        self.loss_L1_img = self.criterionL1(self.fake_B_final, self.img_B) * self.opt.lambda_L1
        self.loss_L1_seg = self.criterionL1(self.fake_D_final, self.img_D) * self.opt.lambda_L1_seg
