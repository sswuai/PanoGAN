...
class XForkModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            self.netD = networks.define_D(2 * opt.input_nc, opt.ndf, opt.netD,
    ...
    def backward_D(self):
        ...
        pred_fake_AB = self.netD(fake_AB.detach())  # Fake; stop backprop to the generator by detaching fake_B
        self.loss_D_fake = self.criterionGAN(pred_fake_AB, 0)
        # Real
        real_AB = torch.cat((self.img_A, self.img_B), 1)
        pred_real_AB = self.netD(real_AB)
        self.loss_D_real = self.criterionGAN(pred_real_AB, 0.9)
    ...
    def backward_G(self):
        ...
        fake_AB = torch.cat((self.img_A, self.fake_B),
                        1)  # we use conditional GANs; we need to feed both input and output to the discriminator
        pred_fake_AP = self.netD(fake_AB)
        self.loss_G_GAN = self.criterionGAN(pred_fake_AP, 0.9)
        # Second, G(A) = B
        self.loss_G_L1_B = self.criterionL1(self.fake_B, self.img_B) * self.opt.lambda_L1
        self.loss_G_L1_D = self.criterionL1(self.fake_D, self.img_D) * self.opt.lambda_L1
