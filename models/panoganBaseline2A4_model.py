...
"""
class panoganBaseline2A4MOdel(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        parser.set_defaults(norm='batch', netG='panoganBaseline2A4', dataset_mode='panoaligned5')
        if is_train:
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D', 'D_imgseg_fake', 'D_imgseg_real',
                           'G', 'GAN_imgseg', 'L1_img', 'L1_seg']
        ...
        if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            self.netD1 = networks.define_D(opt.input_nc + opt.output_nc * 2, opt.ndf, opt.netD,
                                           opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
            self.netD2 = networks.define_D(opt.input_nc + opt.output_nc * 2, opt.ndf, opt.netD,
                                           opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
            self.netD3 = networks.define_D(opt.input_nc + opt.output_nc * 2, opt.ndf, opt.netD,
                                           opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
            self.netD4 = networks.define_D(opt.input_nc + opt.output_nc * 2, opt.ndf, opt.netD,
                                           opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
    ...
    def forward(self):
        ...
        self.fake_BD123 = torch.cat((self.fake_B123, self.fake_D123), 1)
        ...
        self.fake_BD1234 = torch.cat((self.fake_B1234, self.fake_D1234), 1)
    ...
    def backward_D(self):
        ...
        real_region_BD = torch.cat((real_region_B, real_region_D),1)
        cond_input = self.input1
        fake_SB = torch.cat((cond_input, self.fake_BD1), 1)
        pred_fake = self.netD1(fake_SB.detach())
        self.loss_D_img_fake_1 = self.criterionGAN(pred_fake, self.false_label)
        real_SB = torch.cat((cond_input, real_region_BD), 1)
        ...
        real_region_BD = torch.cat((real_region_B, real_region_D), 1)
        cond_input = self.input12
        fake_SB = torch.cat((cond_input, self.fake_BD12), 1)
        ...
        real_region_BD = torch.cat((real_region_B, real_region_D), 1)
        cond_input = self.input123
        fake_SB = torch.cat((cond_input, self.fake_BD123), 1)
        ...
        real_region_BD = torch.cat((real_region_B, real_region_D), 1)
        cond_input = self.input1234
        fake_SB = torch.cat((cond_input, self.fake_BD1234), 1)
    ...
    def backward_G(self):
        ...
        self.loss_GAN_imgseg_1 = self.criterionGAN(pred_fake, self.real_label)

        # --------G2--------------------
        cond_input = self.input12
        fake_SB = torch.cat((cond_input, self.fake_BD12), 1)
        pred_fake = self.netD2(fake_SB)
        self.loss_GAN_imgseg_12 = self.criterionGAN(pred_fake, self.real_label)

        # --------D3--------------------
        cond_input = self.input123
        fake_SB = torch.cat((cond_input, self.fake_BD123), 1)
        pred_fake = self.netD3(fake_SB)
        self.loss_GAN_imgseg_123 = self.criterionGAN(pred_fake, self.real_label)

        # --------D4--------------------
        cond_input = self.input1234
        fake_SB = torch.cat((cond_input, self.fake_BD1234), 1)
        pred_fake = self.netD4(fake_SB)
        self.loss_GAN_imgseg_1234 = self.criterionGAN(pred_fake, self.real_label)

        # ---------------- total GAN loss -----------------------------------
        self.loss_GAN_imgseg = (
                self.loss_GAN_imgseg_1 + self.loss_GAN_imgseg_12 +
                self.loss_GAN_imgseg_123 + self.loss_GAN_imgseg_1234)
        ...
        self.loss_G = self.loss_GAN_imgseg + self.loss_L1
