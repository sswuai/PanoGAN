...
panoganBaseline2A1
    ...
    3. GAN loss for image only, input is concat(previous_input, fake_img/real_img)
    4. L1 loss for image and seg
"""
class panoganBaseline2A1MOdel(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        parser.set_defaults(norm='batch', netG='panoganBaseline2A1', dataset_mode='panoaligned4')
        if is_train:
