...
class panoganBaseline5bModel(BaseModel):
    """
    ...
    def __init__(self, opt):
        ...
        self.netD_img = networks.define_D(opt.input_nc, opt.ndf, opt.netD,
                                       opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)

        self.netD_seg = networks.define_D(opt.input_nc, opt.ndf, opt.netD,
                                       opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
    ...
    def forward(self):
        ...
        for i in range(0, self.loop_count):
            ...
            _, _, disc_out_img = self.netD_img(fake_B)  # Fake; stop backprop to the generator by detaching fake_B

            # ---------seg loss ------------------------
            # fake
            #fake_AD = torch.cat((self.img_A, fake_D), 1)  # we use conditional GANs; we need to feed both input and output to the discriminator
            _, _, disc_out_seg = self.netD_seg(fake_D)  # Fake; stop backprop to the generator by detaching fake_B
    ...
    def backward_D(self):
        ...
        # ---------D loss ------------------------
            ...
            pred_fake_img, embed_fake_img, _ = self.netD_img(
                self.fake_B[j].detach())  # Fake; stop backprop to the generator by detaching fake_B
            # real image
            pred_real_img, embed_real_img, _ = self.netD_img(self.img_B)
            # fake seg
            pred_fake_seg, embed_fake_seg, _ = self.netD_seg(
                self.fake_D[j].detach())  # Fake; stop backprop to the generator by detaching fake_B
            # real seg
            pred_real_seg, embed_real_seg, _ = self.netD_seg(self.img_D)
            # fuse prediction map
    ...
    def backward_G(self):
        ...
        for j in range(self.loop_count):
            ...
            # fake img
            pred_fake_img, embed_fake_img, _ = self.netD_img(
                self.fake_B[j])  # Fake; stop backprop to the generator by detaching fake_B
            # real image
            pred_real_img, embed_real_img, _ = self.netD_img(self.img_B)
            # fake seg
            pred_fake_seg, embed_fake_seg, _ = self.netD_seg(
                self.fake_D[j])  # Fake; stop backprop to the generator by detaching fake_B
            # real seg
            pred_real_seg, embed_real_seg, _ = self.netD_seg(self.img_D)
            #fuse predict map
            pred_embed_fake = [pred_fake_img, embed_fake_img, pred_fake_seg, embed_fake_seg]
