...
panoganBaseline2A2
    ...
    3. GAN loss for image only, input is concat(real_seg, fake_img/real_img)
    4. L1 loss for image and seg
"""
class panoganBaseline2A2MOdel(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        parser.set_defaults(norm='batch', netG='panoganBaseline2A2', dataset_mode='panoaligned4')
        if is_train:
    ...
    def forward(self):
        ...
        fakeBD = self.netG1(self.input1)
        self.fake_B1 = fakeBD[:,0:3,:,:]
        self.fake_D1 = fakeBD[:,3:6,:,:]

        self.input12 = torch.cat((self.fake_B1, self.real_A[:, :, :, 256 * 1:256 * 2]),3)
        fakeBD = self.netG2(self.input12)
        self.fake_B12 = fakeBD[:, 0:3, :, :]
        self.fake_D12 = fakeBD[:, 3:6, :, :]

        self.input123 = torch.cat((self.fake_B12, self.real_A[:, :, :, 256 * 2:256 * 3]), 3)
        fakeBD = self.netG3(self.input123)
        self.fake_B123 = fakeBD[:, 0:3, :, :]
        self.fake_D123 = fakeBD[:, 3:6, :, :]

        self.input1234 = torch.cat((self.fake_B123, self.real_A[:, :, :, 256 * 3:256 * 4]), 3)
        fakeBD = self.netG4(self.input1234)
        self.fake_B1234 = fakeBD[:, 0:3, :, :]
        self.fake_D1234 = fakeBD[:, 3:6, :, :]
    ...
    def backward_D(self):
        ...
        fake_SB = torch.cat((cond_input, self.fake_B12), 1)
        ...
        fake_SB = torch.cat((cond_input, self.fake_B123), 1)
        ...
        fake_SB = torch.cat((cond_input, self.fake_B1234), 1)
    ...
    def backward_G(self):
        ...
        self.loss_GAN_img1 = self.criterionGAN(pred_fake, self.real_label)

        # --------D2--------------------
        cond_input = self.input12
        fake_SB = torch.cat((cond_input, self.fake_B12), 1)
        pred_fake = self.netD2(fake_SB)
        self.loss_GAN_img12 = self.criterionGAN(pred_fake, self.real_label)

        # --------D3--------------------
        cond_input = self.input123
        fake_SB = torch.cat((cond_input, self.fake_B123), 1)
        pred_fake = self.netD3(fake_SB)
        self.loss_GAN_img123 = self.criterionGAN(pred_fake, self.real_label)

        # --------D4--------------------
        cond_input = self.input1234
        fake_SB = torch.cat((cond_input, self.fake_B1234), 1)
        pred_fake = self.netD4(fake_SB)
        self.loss_GAN_img1234 = self.criterionGAN(pred_fake, self.real_label)

        # ---------------- total GAN loss -----------------------------------
        self.loss_GAN = (
                self.loss_GAN_img1 + self.loss_GAN_img12 +
                self.loss_GAN_img123 + self.loss_GAN_img1234)
