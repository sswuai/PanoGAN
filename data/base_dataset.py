...
def get_params(opt, size):
    ...
    return {'crop_pos': (x, y), 'flip': flip, 'new_h': new_h, 'new_w': new_w}
...
def get_transform_pano(opt, params=None, grayscale=False, method=Image.BICUBIC, convert=True):
    ...
    if 'resize' in opt.preprocess:
        if params is None:
            osize = [opt.load_size, opt.load_size*4]
        else:
...
def get_transform_pano_new(opt, params=None, grayscale=False, method=Image.BICUBIC, convert=True, aerial=False):
    transform_list = []
