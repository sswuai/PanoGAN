...
class PanoAlignedDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        B = ABC.crop((w1, 0, w1 + w2, h)).resize((self.opt.load_size * 4, self.opt.load_size), Image.BICUBIC)
        D = ABC.crop((w1 + w2, 0, w1 + w2 + w2, h)).resize((self.opt.load_size * 4, self.opt.load_size), Image.BICUBIC)

        A = transforms.ToTensor()(A)
        B = transforms.ToTensor()(B)
        D = transforms.ToTensor()(D)
        w_offset = random.randint(0, max(0, (self.opt.load_size - self.opt.crop_size - 1) * 4))
        ...
        B = B[:, h_offset:h_offset + self.opt.crop_size, w_offset:w_offset + self.opt.crop_size * 4]
        D = D[:, h_offset:h_offset + self.opt.crop_size, w_offset:w_offset + self.opt.crop_size * 4]
